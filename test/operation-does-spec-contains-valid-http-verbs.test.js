...
beforeAll(async () => {
  linter = await linterForRule("tdp-does-spec-contains-valid-http-verbs");
  return linter;
});

test("does-spec-contains-valid-http-verbs should return error since spec contains invalid http verbs", () => {
  const oasDoc = {
    openapi: "3.0.0",
    "paths": {
      "/apis": {
  ...
  return linter.run(oasDoc).then((results) => {
    ...
    expect(results[0].message).toBe("list is not a valid HTTP verb.");
